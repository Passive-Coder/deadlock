...
def dataset(seed):
    ...
    return [
        {"id": i + 1, "region": ["North", "South", "East", "West"][i % 4], "amount": rng.randint(10, 200)}
        for i in range(60)
    ]
...
def summary(rows):
    return [
        {"region": region, "total": sum(r["amount"] for r in rows if r["region"] == region)}
...
def validate(path: Path, kind, rows):
    try:
        ...
        if kind == "summary":
            ...
            valid = all(
                ...
                for x in [
                    ...
                    "Regional summary",
                    "Dataset overview",
                    f'data-total="{sum(r["amount"] for r in rows)}"',
                ]
            )
            valid = valid and all(
                f"<td>{r['region']}</td><td>{r['total']}</td>" in text for r in summary(rows)
            )
        elif kind == "package":
        ...
        return {
            ...
            "sha256": digest(raw),
            "size": len(raw),
            "error": None if valid else "Content validation failed",
        }
...
def publish(directory, worker, rows):
    ...
    if kind == "summary":
        ...
        temp.write_text(
            f'<!doctype html><html lang="en"><meta charset="utf-8"><title>Regional report</title>'
        ...
        with zipfile.ZipFile(temp, "w", zipfile.ZIP_DEFLATED) as archive:
            ...
            archive.writestr(
                "manifest.json", json.dumps({"input_sha256": digest(csv_bytes(rows)), "rows": len(rows)})
            )
    else:
