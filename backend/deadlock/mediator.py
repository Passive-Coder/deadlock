...
TOOL_SCHEMA = {
    ...
    "properties": {
        "tool": {"type": "string", "enum": ["inspect_candidate", "propose_recovery", "unresolved"]},
    ...
    async def choose(self, evidence, timeout):
        if self.settings.mediator == "openai":
            if not os.getenv("OPENAI_API_KEY") or not os.getenv("OPENAI_MODEL"):
                raise Rejection(
                    "MODEL_UNAVAILABLE", "Configure OPENAI_API_KEY and OPENAI_MODEL to use the API mediator"
                )
                        ...
                        "tools": [
                            {
                                "type": "function",
                                "name": "recovery_tool",
                                "description": "Inspect recovery evidence or propose a validated recovery",
                                "strict": True,
        ...
        if self.settings.mediator != "codex" or not shutil.which("codex"):
            raise Rejection(
                "MODEL_UNAVAILABLE", "Install and sign into Codex, or configure the OpenAI API mediator"
            )
        with tempfile.TemporaryDirectory(prefix="deadlock-mediator-") as directory:
            ...
            try:
                ...
                if proc.returncode != 0 or not output.exists():
                    raise Rejection(
                        "MODEL_ERROR",
                        f"Codex mediator exited without a valid result (code {proc.returncode}); check Codex login and model access",
                    )
                actual_model = re.search(r"(?m)^model:\s*(\S+)", stderr.decode(errors="replace"))
                return json.loads(output.read_text()), os.getenv("OPENAI_MODEL") or (
                    actual_model.group(1) if actual_model else "Codex CLI default (model not reported)"
                )
            finally:
...
class Mediator:
    ...
    def trace(self, name, arguments, result):
        ...
        incident["trace"].append(
            {"time": time.time(), "tool": name, "arguments": arguments, "result": result}
        )
    ...
    async def investigate(self, force=False):
        ...
        try:
            while incident["attempts"] < 2:
                ...
                if incident["attempts"] == 0:
                    self.trace(
                        ...
                        {"incident_id": incident["id"]},
                        {
                            ...
                            "snapshot_id": runner.persisted["id"],
                ...
                if run["strategy"] == "live":
                    while True:
                        ...
                        incident["model_calls"] += 1
                        runner.event(
                            "model.requested",
                            "Requested a live mediator decision",
                            provider=runner.settings.mediator,
                        )
                        try:
                            decision, model_name = await self.model.choose(
                                {
                                    "incident": {"id": incident["id"], "cycle": incident["key"]},
                                    "candidates": candidates,
                        ...
                        runner.event(
                            "model.responded",
                            "Received a live mediator tool request",
                            model=model_name,
                            decision=decision,
                        )
                        if not isinstance(decision, dict) or set(decision) != set(TOOL_SCHEMA["required"]):
                            raise Rejection(
                                "INVALID_MODEL_RESPONSE", "Mediator returned an invalid tool envelope"
                            )
                        if not isinstance(decision["rationale"], str) or not isinstance(
                        ...
                        if not selected:
                            raise Rejection(
                                "INVALID_CANDIDATE",
                                "Model selected a candidate outside the returned allowlist",
                            )
                        ...
                        if decision["tool"] != "propose_recovery" or selected["id"] not in inspected:
                            raise Rejection(
                                ...
                                "Inspect the selected candidate before proposing recovery",
                            )
                        rationale, evidence = decision["rationale"], decision["evidence_ids"]
                        break
                else:
                    incident["model"] = (
                        "deterministic least-cost policy"
                        if run["strategy"] != "restart_all"
                        else "deterministic restart-all baseline"
                    )
                    if not candidates:
                        runner.unresolved(
                            "No cycle participant declares a currently eligible recovery capability"
                        )
                        return
                ...
                self.trace(
                    "propose_recovery",
                    {"candidate_id": selected["id"], "rationale": rationale},
                    {"plan_id": plan["id"]},
                )
                ...
                try:
                    result = runner.execute(
                        plan["id"], uid(), fail_before_commit=run["scenario"] == "failure"
                    )
                    ...
                    self.trace(
                        "execute_recovery",
                        {"plan_id": plan["id"]},
                        {"rejected": exc.code, "reason": str(exc)},
                    )
            ...
            if (
                ...
                and incident["status"] not in {"RESOLVED", "UNRESOLVED"}
            ):
